...
public final class Fiber {
	...
	static void ds() {
		for (StackTraceElement ste : new Exception().getStackTrace()) {
			...
			if (cl.startsWith("kilim.Worker") || meth.equals("go")
					|| meth.equals("ds"))
				continue;
			String line = ste.getLineNumber() < 0 ? "" : ":"
					+ ste.getLineNumber();
	...
	void togglePause() {
		...
		if (curState == null) {
			...
			assert curState == PAUSE_STATE : "togglePause: Expected PAUSE_STATE, instead got: iStack == "
					+ iStack + ", state = " + curState;
	...
	public String toString() {
		...
		for (int i = 0; i < stateStack.length; i++) {
			...
			if (st != null) {
				sb.append(st.getClass().getName()).append('[').append(i)
						.append("]: ");
