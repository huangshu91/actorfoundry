 ...
 * <dl>
 * <li>
 * We create BasicBlocks whenever we encounter a label (in a linear scanning of
 * a method's instructions. Some labels are meant for catch handlers and debug
 * (line number) information only; they are not the target of a branching
 * instruction, but we don't know that in the first pass. We coalesce those
 * BasicBlocks that merely follow another, provided the preceding BB is the only
 * preceder. Note that blocks connected with a GOTO can't coalesce because they
 * are not likely to be contiguous, even if they obey the constraint of a single
 * edge. We also don't coalesce blocks starting with a pausable method
 * invocation with their predecessor, because we need these blocks to tell us
 * about downstream usage of local vars to help us generate optimal
 * continuations.</li>
 * 
 * 
 * <li>All catch handlers that intersect a basic block are treated as successors
 * to the block, for the purposes of liveness analysis.
 * 
...
public class BasicBlock implements Comparable<BasicBlock> {
	...
	/*
	 * Set for the entry point to a subroutine that contains a pausable method.
	 * The entry point is the target of a JSR instruction.
	 */
	 ...
	 * information is available). The constraints are as follows: 1. A transfer
	 * of control instruction must be the last instruction. It may also be the
	 * first (and only) instruction 2. A labeled instruction must be the first
	 * instruction in a BB. It may optionally be the last (and only) instruction
	 * 3. A pausable method is treated like a labeled instruction, and is given
	 * a label if there isn't one already. Constraint 2 applies.
	 */
	 ...
	 * marks out acontiguous range of instructions. Most compilers would have
		...
		while (true) {
			if (successors.size() == 1) {
				...
				if (succ.numPredecessors == 1 && lastInstruction() != GOTO
					...
					// remains 0. We'll let the outer driver
					// loop to
	 ...
	 * verifier which tracks the flow of types, this method tracks values, which
	 * allows us to track types as well as the flow of constant values and set
	 * the stage for SSA-style optimizations.
	 */
	void interpret() {
		Value v, v1, v2, v3, v4;
		...
		try {
			for (i = startPos; i <= endPos; i++) {
				...
				case SALOAD:
					...
					frame.push(Value.make(i, TypeDesc.getComponentType(v
							.getTypeDesc()))); // push
					// component
					// of
					// array
				...
				case RETURN:
					...
					if (frame.stacklen != 0) {
						throw new InternalError(
								"stack non null at method return");
					}
					break;

				case GETSTATIC:
					canThrowException = true;
					v = Value.make(i, TypeDesc
							.getInterned(((FieldInsnNode) ain).desc));
					frame.push(v);
					break;
				...
				case NEW:
					canThrowException = true;
					v = Value.make(i, TypeDesc
							.getInterned(((TypeInsnNode) ain).desc));
					frame.push(v);
					break;
				...
				case NEWARRAY:
					...
					default:
						throw new InternalError(
								"Illegal argument to NEWARRAY: " + atype);
					}
				...
				case ANEWARRAY:
					canThrowException = true;
					frame.popWord();
					componentType = TypeDesc
							.getInterned(((TypeInsnNode) ain).desc);
					v = Value
							.make(i, TypeDesc.getInterned("[" + componentType));
					frame.push(v);
					break;
	...
	public void chooseCatchHandlers(ArrayList<Handler> handlerList) {
		for (Handler h : handlerList) {
			if (this == h.catchBB) {
				...
				caughtExceptionType = TypeDesc
						.getInterned((h.type == null ? THROWABLE_CLASS : h.type));
			} else {
				Range ri = Range.intersect(startPos, endPos, h.from, h.to);
				if (ri != null) {
					handlers
							.add(new Handler(ri.from, ri.to, h.type, h.catchBB));
				}
			}
		}
	}
	 ...
	 * ok with it: (a) looping back to itself (b) encountering xRETURN in a
	 * subroutine
	 * 
	...
	ArrayList<BasicBlock> inline() throws KilimException {
		...
		successors.clear();
		// first pass
		targetBB.dupBBAndLabels(isPausableSub, bbCopyMap, labelCopyMap,
				returnToBB);
		addSuccessor(bbCopyMap.get(targetBB));
		// second pass
		return dupCopyContents(isPausableSub, targetBB, returnToBB, bbCopyMap,
				labelCopyMap);
	}
	...
	static ArrayList<BasicBlock> dupCopyContents(boolean deepCopy,
		...
		ArrayList<BasicBlock> newBBs = new ArrayList<BasicBlock>(targetBB
				.getSubBlocks().size());
		for (BasicBlock orig : targetBB.getSubBlocks()) {
			...
			if (deepCopy) {
				...
				AbstractInsnNode lastInsn = (AbstractInsnNode) instructions
						.get(orig.endPos);
				Label dupLabel;
				...
				if (lastInsn instanceof JumpInsnNode) {
					...
					if (lastInsn.getOpcode() != JSR) {
						...
						lastInsn = new JumpInsnNode(lastInsn.getOpcode(),
								dupLabel);
					}
					...
					assert dupLabel != null;
					lastInsn = new TableSwitchInsnNode(tsin.min, tsin.max,
							dupLabel, labels);
	...
	/*
	 * Invoked on the subroutine entry point's BB. Returns all the BBs linked to
	 * it.
	 */
				...
				for (BasicBlock succ : b.successors) {
					if (succ == this) {
						throw new KilimException(
								"JSRs looping back to themselves are not supported");
					}
	...
	public String toString() {
		...
		sb.append("\n========== BB #").append(id).append("[").append(
				System.identityHashCode(this)).append("]\n");
		sb.append("method: ").append(this.flow.name).append("\n");
		sb.append("start = ").append(startPos).append(",end = ").append(endPos)
				.append('\n').append("Successors:");
		if (successors.isEmpty())
			...
			for (int i = 0; i < successors.size(); i++) {
				...
				sb.append(" ").append(succ.id).append("[").append(
						System.identityHashCode(succ)).append("]");
			}
		}
		sb.append("\nHandlers:");
		if (handlers.isEmpty())
	...
	/*
	 * If any BB belonging to a subroutine makes a pausable block, it taints all
	 * the blocks within the subroutine's purview as PAUSABLE_SUB
	 */
	...
	public boolean isGetCurrentTask() {
		...
		if (ain.getOpcode() == INVOKESTATIC) {
			...
			return min.owner.equals(TASK_CLASS)
					&& min.name.equals("getCurrentTask");
		}
		return false;
	}
