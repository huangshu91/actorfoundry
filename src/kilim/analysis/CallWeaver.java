...
public class CallWeaver {
	...
	/**
	 * The canoncial name of the state class responsible for storing the state
	 * (@see kilim.State)
	 */
	...
	void genRewind(MethodVisitor mv) {
		...
		for (spos = 0; spos < numBottom; spos++) {
			...
			if (v.isConstant()) {
				mv
						.visitInsn(VMType.constInsn[VMType.toVmType(v
								.getTypeDesc())]);
			} else {
		...
		if (!isStaticCall()) {
			...
			if (ivar != -1) {
				...
				mv.visitMethodInsn(INVOKEVIRTUAL, FIBER_CLASS, "getCallee",
						"()Ljava/lang/Object;");
				checkcast(mv, v);
			}
			spos++;
	...
	private void genSave(MethodVisitor mv, Label saveLabel) {
		...
		for (; i >= 0; i--) {
			...
			if (vi == null) {
				...
				mv.visitFieldInsn(PUTFIELD, stateClassName, vi.fieldName, vi
						.fieldDesc());
	...
	private void genRestore(MethodVisitor mv, Label restoreLabel) {
		...
		for (int i = 0; i < numBottom; i++) {
			...
			if (v.isConstant()) {
				...
				if (vi.var == -1) {
					loadVar(mv, TOBJECT, stateVar);
					mv.visitFieldInsn(GETFIELD, stateClassName, vi.fieldName,
							vi.fieldDesc());
					checkcast(mv, v);
				} else {
		...
		for (; i < len; i++) {
			...
			if (v.isConstant()) {
				...
				if (vi.var == i) {
					...
					mv.visitFieldInsn(GETFIELD, stateClassName, vi.fieldName,
							vi.fieldDesc());
					checkcast(mv, v); // don't need to do this in the constant
										// case
				} else {
					// It is a duplicate of another var. No need to load this
					// var from stack
					assert vi.var < i;
					loadVar(mv, vi.vmt, vi.var);
				}
	...
	private String createStateClass() {
		return valInfoList.size() == 0 ? STATE_CLASS : methodWeaver
				.createStateClass(valInfoList);
	}
...
class VMType {
	...
	static final int[] constInsn = { ACONST_NULL, ICONST_0, LCONST_0, DCONST_0,
			FCONST_0 };
