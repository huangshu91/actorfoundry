...
public class ClassWeaver {
	...
	private void weave() throws KilimException {
		...
		if (classFlow.isPausable() && needsWeaving()) {
			...
			accept(cw);
			addClassInfo(new ClassInfo(classFlow.getClassName(), cw
					.toByteArray()));
		}
	}
	...
	private void accept(final ClassVisitor cv) {
		...
		cv.visit(cf.version, cf.access, cf.name, cf.signature, cf.superName,
				interfaces);
		// visits source
		...
		if (cf.outerClass != null) {
			cv.visitOuterClass(cf.outerClass, cf.outerMethod,
					cf.outerMethodDesc);
		}
		...
		n = cf.invisibleAnnotations == null ? 0 : cf.invisibleAnnotations
				.size();
		for (i = 0; i < n; ++i) {
		...
		/*
		 * Mark this class as "processed" by adding an annotation, so that we
	...
	String createStateClass(ValInfoList valInfoList) {
		...
		ClassWriter cw = new ClassWriter(false);
		cw.visit(V1_1, ACC_PUBLIC | ACC_FINAL, className, null, "kilim/State",
				null);
