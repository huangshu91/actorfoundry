...
public class Detector {
	...
	/**
	 * @return one of METHOD_NOT_FOUND, PAUSABLE_METHOD_FOUND,
	 *         METHOD_NOT_PAUSABLE
	 */
	...
	public static int getPausableStatus(String className, String methodName,
		...
		try {
			...
			if (m != null) {
				ret = (m.isAnnotationPresent(pausable.class) || m
						.isAnnotationPresent(message.class)) ? PAUSABLE_METHOD_FOUND
						: METHOD_NOT_PAUSABLE;
			}
	...
	public static Method findMethodInHierarchy(Class cl, String methodName,
		...
		for (Method om : cl.getDeclaredMethods()) {
			if (om.getName().equals(methodName)
					&& Type.getMethodDescriptor(om).equals(desc)) {
				return om;
			}
		}
	...
	public static int _getPausableStatus(String className, String methodName,
		...
		try {
			...
			ret = (m == null) ? METHOD_NOT_FOUND
					: (m.isAnnotationPresent(pausable.class) || m
							.isAnnotationPresent(message.class)) ? PAUSABLE_METHOD_FOUND
							: METHOD_NOT_PAUSABLE;
		} catch (ClassNotFoundException cnfe) {
			ret = METHOD_NOT_FOUND;
		}
		System.out.println("Detector : " + className + methodName + desc
				+ " -- " + statusToStr(ret));
		return ret;
	}
	...
	private static void initDontCheckList() {
		try {
			...
			if (pkgs != null) {
				...
				System.arraycopy(pkgList, pkgList.length,
						STANDARD_DONT_CHECK_LIST, 0,
						STANDARD_DONT_CHECK_LIST.length);
			}
