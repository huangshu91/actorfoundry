...
public class Frame {
	...
	/**
	 * Merge the local variables and stack from the incoming frame into the
	 * current frame.
	 * 
	 * @param inframe
	 *            -- incoming frame
	 * @param localsOnly
	 *            -- true for exception handlers, because the stack is cleared.
	 * @param usage
	 *            -- Only those locals are merged that are deemed live (@see
	 *            Usage#isLiveIn(int))
	 * @return this, if the merge didn't change anything or a new Frame if the
	 *         operation changed a slot on the stack or a local variable
	 */
	...
	public Value getLocal(int local, int opcode) {
		...
		throw new AssertionError("Expected " + expected + " in local# " + local
				+ ", got " + desc);
	}
	...
	public Value pop() {
		try {
			...
			throw new RuntimeException(
					"Verify error. Expected word in stack, but stack is empty");
		}
	}
