...
public class MethodWeaver {
	...
	public void accept(ClassVisitor cv) {
		...
		MethodVisitor mv = cv.visitMethod(mf.access, mf.name, desc, sig,
				exceptions);
		if (!mf.isAbstract()) {
			if (mf.isPausable()) {
				accept(mv);
	...
	private void visitAttrs(MethodVisitor mv) {
		...
		n = mf.invisibleAnnotations == null ? 0 : mf.invisibleAnnotations
				.size();
		for (i = 0; i < n; ++i) {
	 ...
	 */
	private void genException(MethodVisitor mv, BasicBlock bb,
	...
	void visitTryCatchBlocks(MethodVisitor mv) {
		...
		for (Handler h : allHandlers) {
			mv.visitTryCatchBlock(mf.getLabelAt(h.from), mf
					.getOrCreateLabelAtPos(h.to + 1), h.catchBB.startLabel,
					h.type);
		}
	}
