...
/**
 * Each BasicBlock owns one instance of Usage. This class maintains, in essence,
 * three vectors of booleans, indexed by the local variable number. Since it is
 * <i>very</i> rare for a method to have more than 31 local variables, the
 * vectors are represented by int bitmaps. For more than this, the basic block
 * creates an instance of BigUsage that is functionally identical (TODO)
 * 
 * @see BasicBlock
 */
public class Usage {
	 ...
	 * written. The bit vector as a whole represents the set of vars that the BB
	 * needs from its predecessors.
	 */
	private BitSet use;
	...
	public void read(int var) {
		assert var < nLocals : "local var num=" + var + " exceeds nLocals = "
				+ nLocals;
		if (!def.get(var)) {
	...
	/**
	 * Called to coalesce a successor's usage into the current BB. Important:
	 * This should be called before live variable analysis begins, because we
	 * don't bother merging this.in.
	 */
