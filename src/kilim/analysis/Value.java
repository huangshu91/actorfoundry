...
/**
 * A SSA value that represents all objects produced at a particular location in
 * the code. Value objects are used by dataflow analysis (@see BasicBlock)
 * 
 */
public class Value {
	...
	private Value(int aPos, String aDesc, Object aConst) {
		...
		typeDesc = aDesc;
		constVal = aConst;
		// System.out.println("V[" + aPos + ":" + aDesc + ((aConst == NO_VAL) ?
		// "" : (": " + aConst)) + "]");
	}

	private Value(int newNumSites, int[] newSites, String newType,
			Object newConst) {
		...
		constVal = newConst;
		/*
		 * //debug StringBuilder sb = new StringBuilder(80); sb.append("V[");
		 * for (int i = 0; i < newNumSites; i++) { if (i > 0) sb.append(",");
		 * sb.append(newSites[i]); } sb.append(":").append(newType).append(":");
		 * if (newConst != NO_VAL) {
		 * sb.append(": ").append(newConst.toString()); } sb.append("]");
	...
	public boolean equals(Object obj) {
		// TODO FIXME : This is WRONG. Two values can be created at the same
		// site when
		// entering a method (all incoming parameter values are given location
		// 0).
