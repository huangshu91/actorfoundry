...
public class FlowAnalyzer {
	public static void main(String[] args) throws Exception {
		if (args.length == 0) {
			System.err
					.println("Usage <class name | jar file name> [methodName]");
			System.exit(1);
		}
		String name = args[0];
	...
	private static void analyzeClass(String className) {
		try {
			...
			pn("Class: " + className);
			System.out.flush();
			ArrayList<MethodFlow> flows = new ClassFlow(className)
					.analyze(true);
	...
	private static String uniqueItems(BasicBlock bb, Frame f, Usage u,
			int nStack) {
		...
		// create canonical sig. Convert types to one of 'O', 'I', 'F', 'L', 'D'
		// and
		// put in sorted order
		...
		sb.append("\nState signature: ").append(
				set.size() == 0 ? "None" : new String(sig));
		return sb.toString();
	}
