...
public class ContinuationSchedulerWithWaitNotify implements Scheduler, Runnable {
	...
	public void scheduleThread(final Task toSchedActor)
		...
		if (toSchedActor != null) {
			...
			Object oMonitor = waitingWorkerQueue.dequeue();
			if (oMonitor != null) {
	...
	private Queue<ObjectMonitor> waitingWorkerQueue = new Queue<ObjectMonitor>(
			INIT_WORKER_POOL_SIZE);
	...
	class WorkerThread extends Thread {
		...
		// Integer triState = new Random().nextInt(3);
		...
		public void run() {
			...
			while (true) {
				...
				nextThread = privateSchedule.dequeue();
				if (nextThread != null) {
					...
					try {
						...
						// }
						// else {
						waitingWorkerQueue.enqueue(monitor);
