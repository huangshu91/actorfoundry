...
public class Queue<T> implements Cloneable, Serializable {
	...
	/**
	 * This function allows the evaluation of a predicate on each element of the
	 * Queue. We use this feature to allow searches. The predicate takes the
	 * form of a class which implements the <em>QueueSearch</em> interface. This
	 * interface defines the single function <em>queueEvalPred</em> which takes
	 * an object as an argument and returns true or false.
	 * 
	 * @param <b>predObj</b> An instance of a class which implements the
	 *        <em>QueueSearch</em> interface.
	 * @return An array of objects corresponding to the elements of the queue
	 *         which satisfy the predicate. An array of length 0 is returned if
	 *         no objects satisfy the predicate.
	 */
	synchronized public Object[] search(QueueSearch searchInstance) {

		// If empty, return nothing exciting
		if (empty())
			return zeroArray;

		// Otherwise, build a potential return vector and fill it with
		// matching elements
	...
	synchronized public boolean remove(T rem) {
		...
		// Log.println("<Queue.remove> Searching for element: " + rem);
		...
		if (i == back)
			// PRAGMA [debug,osl.util.Queue] {
			// PRAGMA [debug,osl.util.Queue]
			// Log.println("<Queue.remove> Element not found, returning");
			return false;
		// PRAGMA [debug,osl.util.Queue] } else {
		// PRAGMA [debug,osl.util.Queue]
		// Log.println("<Queue.remove> Element found at position: " + i);
		// PRAGMA [debug,osl.util.Queue] }
